...
SUPER_PRIMITIVES = {'SearchBlock' : lambda input_size, in_channels, out_channels, stride: SearchBlock(input_size, in_channels, out_channels, stride),
                    'SUB_SUPER_PRIMITIVES' : lambda input_size, in_channels, out_channels, stride: SubSearchBlock(input_size, in_channels, out_channels, stride)}
# subkey = ['ir_k5_e3', 'ir_k3_e6_r2', 'ir_k3_s2', 'ir_k5_e6_r2', 'ir_k3_e3_r3', 'ir_k3_e1_r2', 'sep_k3', 'ir_k3_e1', 'ir_k3_e3_r2']
subkey = ['ir_k5_e3', 'ir_k3_e6_r2', 'ir_k3_s2', 'ir_k5_e6_r2', 'ir_k3_e1_r2', 'sep_k3', 'ir_k3_e3_r2']
SUB_SUPER_PRIMITIVES = {}
for key in subkey:
    SUB_SUPER_PRIMITIVES[key] = PRIMITIVES[key]
...
class MixedOp(nn.Module):
    ...
    def __init__(self, input_size, in_channels, out_channels, stride):
        ...
        for key in PRIMITIVES:
            # print(key)
            self._opsKeys.append(key)
            op = PRIMITIVES[key](input_size, in_channels, out_channels, stride)
            self._ops.append(op)

    def forward(self, x, arch_weights):
        # print('arch_weights', arch_weights)
        # print(self._ops)
        return sum(w * op(x) for w, op in zip(arch_weights, self._ops))

class SearchBlock(nn.Module):
    def __init__(self, input_size, _in_channels, _out_channels, stride):
        super(SearchBlock, self).__init__()
        self.cell = MixedOp(input_size, _in_channels, _out_channels, stride)

    def forward(self, x, arch_weights):
        return self.cell(x, arch_weights)

class SubMixedOp(nn.Module):

    def __init__(self, input_size, in_channels, out_channels, stride):
        super(SubMixedOp, self).__init__()
        self._ops = nn.ModuleList()
        self._opsKeys = []
        for key in SUB_SUPER_PRIMITIVES:
            # print(key)
            self._opsKeys.append(key)
            op = SUB_SUPER_PRIMITIVES[key](input_size, in_channels, out_channels, stride)
            self._ops.append(op)

    def forward(self, x, arch_weights):
        return sum(w * op(x) for w, op in zip(arch_weights, self._ops))

class SubSearchBlock(nn.Module):
    def __init__(self, input_size, _in_channels, _out_channels, stride):
        super(SubSearchBlock, self).__init__()
        self.cell = SubMixedOp(input_size, _in_channels, _out_channels, stride)
...
class ConvBNReLU(nn.Module):
    def __init__(self, input_size, in_channels, out_channels, kernel_size, stride, padding, bias, relu_type, bn_type,
                 groups=1, dilation=1):
...
class SepBlock(nn.Module):
    def __init__(self, genotype, input_size, in_channels, out_channels, expansion, stride, kernel_size, groups=1,
                 bn_type='bn'):
        ...
        self.conv1 = ConvBNReLU(input_size, in_channels, in_channels, kernel_size=kernel_size, stride=self.stride,
                                padding=padding, bias=False, relu_type='relu', bn_type=bn_type, groups=in_channels)
        self.conv2 = ConvBNReLU(input_size // stride, in_channels, out_channels, kernel_size=1, stride=1, padding=0,
...
class MBBlock(nn.Module):
    def __init__(self, input_size, in_channels, out_channels, expansion, stride, kernel_size, dilation=1, groups=1,
                 has_se=False, bn_type='bn'):
        ...
        self.conv2 = ConvBNReLU(input_size, mid_channels, mid_channels, kernel_size=kernel_size, stride=self.stride,
                                padding=padding, dilation=dilation,
                                bias=False, relu_type='relu', bn_type=bn_type, groups=mid_channels)
        self.conv3 = ConvBNReLU(input_size // self.stride, mid_channels, out_channels, kernel_size=1, stride=1,
